...
from model import UNet_A,UNet_B,UNet_C
...
#base_size  = img_size // n_num # lateral size of lf_extra ##changed 6/2/2021
base_size  = [config.img_size // n_num,config.img_size // n_num]
...
normalize_mode  = 'constant' if config.model is 'function' else config.normalize_mode
...
class Trainer:   
    ...
    def build_graph(self, use_cpu=True): #changed from use_cpu False to True, 6/2/2021
        ###========================== DEFINE MODEL ============================###

        #tried getting mixed precision to work, use less gpu space for model contruction
        #policy = mixed_precision.experimental.Policy('infer')
        #mixed_precision.experimental.set_policy(policy)
        
        with tf.variable_scope('learning_rate'):
            self.learning_rate = tf.Variable(lr_init, trainable=False)
                
        ##tried changing dtype in placeholders from float32 to bfloat16, tensorlayer didn't allow
        ...
        with tf.device(device_str):
            ...
            elif model_type is 'structure':
                self.net = UNet_A(self.plchdr_lf, n_slices=n_slices, output_size=img_size, is_train=True, reuse=False, name=vars_tag)
            elif 'beta' in model_type :
                self.net = UNet_C(self.plchdr_lf, n_slices=n_slices, output_size=img_size, is_train=True, reuse=False,name=vars_tag,normalize_mode=normalize_mode)
        ...
        #to disable gpu and force tensorflow to use cpu with more RAM
        if use_cpu:
